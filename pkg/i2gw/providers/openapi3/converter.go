...
func (c *resourcesToIRConverter) Convert(storage Storage) (emitterir.EmitterIR, field.ErrorList) {
	...
	for _, spec := range storage.GetResources() {
		...
		for _, httpRoute := range httpRoutes {
			...
			c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("successfully created HTTPRoute \"%v/%v\" from OpenAPI spec \"%v\"", httpRoute.Namespace, httpRoute.Name, spec.Info.Title))
		}
		...
		if referenceGrant := c.buildHTTPRouteBackendReferenceGrant(); referenceGrant != nil {
			...
			c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("successfully created ReferenceGrant \"%v/%v\" from OpenAPI spec \"%v\"", referenceGrant.Namespace, referenceGrant.Name, spec.Info.Title))
		}
		for _, gateway := range gateways {
			...
			c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("successfully created Gateway \"%v/%v\" from OpenAPI spec \"%v\"", gateway.Namespace, gateway.Name, spec.Info.Title))
		}
	}
