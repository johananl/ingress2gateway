...
func newResourcesToIRConverter(notifier *notifications.Notifier) resourcesToIRConverter {
...
func (c *resourcesToIRConverter) convertGateway(gw *istioclientv1beta1.Gateway, fieldPath *field.Path) (*gatewayv1.Gateway, field.ErrorList) {
	...
	for i, server := range gw.Spec.GetServers() {
		...
		if serverPort == nil {
			c.notifier.Notify(notifications.ErrorNotification, fmt.Sprintf("port is nil, path %v", serverFieldPath), gw)
		...
		if serverPort.GetName() != "" {
			c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("ignoring field: %v", portFieldPath.Child("Name")), gw)
		...
		if serverTLS := server.GetTls(); serverTLS != nil {
			...
			case istiov1beta1.ServerTLSSettings_ISTIO_MUTUAL, istiov1beta1.ServerTLSSettings_OPTIONAL_MUTUAL:
				c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("the istio server is ignored as there's no direct translation for this TLS istio protocol: %v", tlsFieldPath.Child("Mode").Key(serverTLSMode.String())), gw)
			...
			if serverTLS.GetHttpsRedirect() {
				c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("ignoring field: %v", tlsFieldPath.Child("HttpsRedirect")), gw)
				klog.Infof("ignoring field: %v", tlsFieldPath.Child("HttpsRedirect"))
			}
			if serverTLS.GetServerCertificate() != "" {
				c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("ignoring field: %v", tlsFieldPath.Child("ServerCertificate")), gw)
				klog.Infof("ignoring field: %v", tlsFieldPath.Child("ServerCertificate"))
			}
			if serverTLS.GetPrivateKey() != "" {
				c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("ignoring field: %v", tlsFieldPath.Child("PrivateKey")), gw)
				klog.Infof("ignoring field: %v", tlsFieldPath.Child("PrivateKey"))
			}
			if serverTLS.GetCaCertificates() != "" {
				c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("ignoring field: %v", tlsFieldPath.Child("CaCertificates")), gw)
				klog.Infof("ignoring field: %v", tlsFieldPath.Child("CaCertificates"))
			}
			if len(serverTLS.GetSubjectAltNames()) > 0 {
				c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("ignoring field: %v", tlsFieldPath.Child("SubjectAltNames")), gw)
				klog.Infof("ignoring field: %v", tlsFieldPath.Child("SubjectAltNames"))
			}
			if serverTLS.GetCredentialName() != "" {
				c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("ignoring field: %v", tlsFieldPath.Child("CredentialName")), gw)
				klog.Infof("ignoring field: %v", tlsFieldPath.Child("CredentialName"))
			}
			if len(serverTLS.GetVerifyCertificateSpki()) > 0 {
				c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("ignoring field: %v", tlsFieldPath.Child("VerifyCertificateSpki")), gw)
				klog.Infof("ignoring field: %v", tlsFieldPath.Child("VerifyCertificateSpki"))
			}
			if len(serverTLS.GetVerifyCertificateHash()) > 0 {
				c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("ignoring field: %v", tlsFieldPath.Child("VerifyCertificateHash")), gw)
				klog.Infof("ignoring field: %v", tlsFieldPath.Child("VerifyCertificateHash"))
			}
			if serverTLS.GetMinProtocolVersion() != 0 {
				c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("ignoring field: %v", tlsFieldPath.Child("MinProtocolVersion")), gw)
				klog.Infof("ignoring field: %v", tlsFieldPath.Child("MinProtocolVersion"))
			}
			if serverTLS.GetMaxProtocolVersion() != 0 {
				c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("ignoring field: %v", tlsFieldPath.Child("MaxProtocolVersion")), gw)
				klog.Infof("ignoring field: %v", tlsFieldPath.Child("MaxProtocolVersion"))
			}
			if len(serverTLS.GetCipherSuites()) > 0 {
				c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("ignoring field: %v", tlsFieldPath.Child("CipherSuites")), gw)
				klog.Infof("ignoring field: %v", tlsFieldPath.Child("CipherSuites"))
			}
		}

		if server.GetBind() != "" {
			c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("ignoring field: %v", serverFieldPath.Child("Bind").Key(server.GetBind())), gw)
		...
		for _, host := range server.GetHosts() {
			...
			if !ok {
				...
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("no namespace specified for host \"%v\", selecting services from all namespaces", host), gw)
	...
	c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("successfully converted to Kubernetes Gateway \"%v/%v\"", gateway.Namespace, gateway.Name), gw)
...
func (c *resourcesToIRConverter) convertHostnames(ctx context.Context, hosts []string, fieldPath *field.Path) []gatewayv1.Hostname {
	...
	for i, host := range hosts {
		...
		if !hostnameRegexp.MatchString(host) {
			c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("ignoring host %s, which is not allowed in Gateway API HTTPRoute, path %v", host, hostsFieldPath), vs)
		...
		if net.ParseIP(host) != nil {
			c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("ignoring host %s, which is an IP address, path %v", host, hostsFieldPath), vs)
...
func (c *resourcesToIRConverter) convertVsHTTPRoutes(virtualService metav1.ObjectMeta, istioHTTPRoutes []*istiov1beta1.HTTPRoute, istioHTTPHosts []string, fieldPath *field.Path) ([]*gatewayv1.HTTPRoute, field.ErrorList) {
	...
	allowedHostnames := c.convertHostnames(c.ctx, istioHTTPHosts, fieldPath)
	...
	for i, httpRoute := range istioHTTPRoutes {
		...
		for j, match := range httpRoute.GetMatch() {
			...
			if match.GetScheme() != nil {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", httpMatchFieldPath.Child("Scheme").Key(match.GetScheme().String())), vs)
				klog.Infof("ignoring field: %v", httpMatchFieldPath.Child("Scheme").Key(match.GetScheme().String()))
			}
			if match.GetAuthority() != nil {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", httpMatchFieldPath.Child("Authority").Key(match.GetAuthority().String())), vs)
				klog.Infof("ignoring field: %v", httpMatchFieldPath.Child("Authority").Key(match.GetAuthority().String()))
			}
			if match.GetPort() != 0 {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", httpMatchFieldPath.Child("Port").Key(fmt.Sprintf("%v", match.GetPort()))), vs)
				klog.Infof("ignoring field: %v", httpMatchFieldPath.Child("Port").Key(fmt.Sprintf("%v", match.GetPort())))
			}
			if len(match.GetSourceLabels()) > 0 {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", httpMatchFieldPath.Child("SourceLabels")), vs)
				klog.Infof("ignoring field: %v", httpMatchFieldPath.Child("SourceLabels"))
			}
			if match.GetIgnoreUriCase() {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", httpMatchFieldPath.Child("IgnoreUriCase")), vs)
				klog.Infof("ignoring field: %v", httpMatchFieldPath.Child("IgnoreUriCase"))
			}
			if len(match.GetWithoutHeaders()) > 0 {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", httpMatchFieldPath.Child("WithoutHeaders")), vs)
				klog.Infof("ignoring field: %v", httpMatchFieldPath.Child("WithoutHeaders"))
			}
			if match.GetSourceNamespace() != "" {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", httpMatchFieldPath.Child("SourceNamespace")), vs)
				klog.Infof("ignoring field: %v", httpMatchFieldPath.Child("SourceNamespace"))
			}
			if match.GetStatPrefix() != "" {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", httpMatchFieldPath.Child("StatPrefix")), vs)
				klog.Infof("ignoring field: %v", httpMatchFieldPath.Child("StatPrefix"))
			}
			if len(match.GetGateways()) > 0 {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", httpMatchFieldPath.Child("Gateways")), vs)
			...
			if matchURI := match.GetUri(); matchURI != nil {
				...
				default:
					c.notifier.Notify(notifications.ErrorNotification, fmt.Sprintf("Unsupported Uri match type, path %v", httpMatchFieldPath.Child("Uri")), vs)
			...
			for header, headerMatch := range match.GetHeaders() {
				...
				default:
					c.notifier.Notify(notifications.ErrorNotification, fmt.Sprintf("Unsupported Headers match type, path %v", httpMatchFieldPath.Child("Headers")), vs)
			...
			for query, queryMatch := range match.GetQueryParams() {
				...
				default:
					c.notifier.Notify(notifications.ErrorNotification, fmt.Sprintf("Unsupported QueryParams match type, path %v", httpMatchFieldPath.Child("QueryParams")), vs)
			...
			if matchMethod := match.GetMethod(); matchMethod != nil {
				...
				default:
					c.notifier.Notify(notifications.ErrorNotification, fmt.Sprintf("Unsupported Method match type, path %v", httpMatchFieldPath.Child("Method")), vs)
		...
		for j, routeDestination := range httpRoute.GetRoute() {
			...
			if routeDestination.GetHeaders() != nil {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", routeDestinationFieldPath.Child("Headers")), vs)
				klog.Infof("ignoring field: %v", routeDestinationFieldPath.Child("Headers"))
			}

			backendObjRef := c.destination2backendObjRef(c.ctx, routeDestination.GetDestination(), virtualService.Namespace, routeDestinationFieldPath)
		...
		if routeRedirect := httpRoute.GetRedirect(); routeRedirect != nil {
			...
			if routeRedirect.GetAuthority() != "" {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", redirectFieldPath.Child("Authority")), vs)
			...
			if _, ok := routeRedirect.GetRedirectPort().(*istiov1beta1.HTTPRedirect_DerivePort); ok {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", redirectFieldPath.Child("DerivePort")), vs)
		...
		if httpRoute.GetDirectResponse() != nil {
			c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", httpRouteFieldPath.Child("DirectResponse")), vs)
			klog.Infof("ignoring field: %v", httpRouteFieldPath.Child("DirectResponse"))
		}
		if httpRoute.GetDelegate() != nil {
			c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", httpRouteFieldPath.Child("Delegate")), vs)
			klog.Infof("ignoring field: %v", httpRouteFieldPath.Child("Delegate"))
		}
		if httpRoute.GetRetries() != nil {
			c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", httpRouteFieldPath.Child("Retries")), vs)
			klog.Infof("ignoring field: %v", httpRouteFieldPath.Child("Retries"))
		}
		if httpRoute.GetFault() != nil {
			c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", httpRouteFieldPath.Child("Fault")), vs)
			klog.Infof("ignoring field: %v", httpRouteFieldPath.Child("Fault"))
		}
		if httpRoute.GetCorsPolicy() != nil {
			c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", httpRouteFieldPath.Child("CorsPolicy")), vs)
		...
		if mirror := httpRoute.GetMirror(); mirror != nil {
			...
			backendObjRef := c.destination2backendObjRef(c.ctx, mirror, virtualService.Namespace, routeDestinationFieldPath)
			if backendObjRef != nil {
				gwHTTPRouteFilters = append(gwHTTPRouteFilters, gatewayv1.HTTPRouteFilter{
		...
		for j, mirror := range httpRoute.GetMirrors() {
			...
			if mirror.GetPercentage() != nil {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", routeDestinationFieldPath.Child("Percentage")), vs)
				klog.Infof("ignoring field: %v", routeDestinationFieldPath.Child("Percentage"))
			}

			backendObjRef := c.destination2backendObjRef(c.ctx, mirror.GetDestination(), virtualService.Namespace, routeDestinationFieldPath)
			if backendObjRef != nil {
				gwHTTPRouteFilters = append(gwHTTPRouteFilters, gatewayv1.HTTPRouteFilter{
					Type: gatewayv1.HTTPRouteFilterRequestMirror,
					RequestMirror: &gatewayv1.HTTPRequestMirrorFilter{
		...
		if httpRoute.GetRewrite() != nil {
			...
			for _, httpRoute := range httpRoutesWithRewrites {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("successfully converted to HTTPRoute \"%v/%v\"", httpRoute.Namespace, httpRoute.Name), vs)
			}
			continue
		}
...
func (c *resourcesToIRConverter) createHTTPRoutesWithRewrite(params createHTTPRouteParams, rewrite *istiov1beta1.HTTPRewrite, fieldPath *field.Path) []*gatewayv1.HTTPRoute {
	...
	if rewrite.GetAuthority() != "" {
		c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", fieldPath.Child("Authority")), vs)
		klog.Infof("ignoring field: %v", fieldPath.Child("Authority"))
	}
	if rewrite.GetUriRegexRewrite() != nil {
		c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", fieldPath.Child("UriRegexRewrite")), vs)
...
func (c *resourcesToIRConverter) convertVsTLSRoutes(virtualService metav1.ObjectMeta, istioTLSRoutes []*istiov1beta1.TLSRoute, fieldPath *field.Path) []*gatewayv1alpha2.TLSRoute {
	...
	for i, route := range istioTLSRoutes {
		...
		for _, destination := range route.GetRoute() {
			backendObjRef := c.destination2backendObjRef(c.ctx, destination.GetDestination(), virtualService.Namespace, tlsRouteFieldPath)
			if backendObjRef != nil {
		...
		for j, match := range route.GetMatch() {
			...
			if len(match.GetDestinationSubnets()) > 0 {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", tlsMatchFieldPath.Child("DestinationSubnets")), vs)
				klog.Infof("ignoring field: %v", tlsMatchFieldPath.Child("DestinationSubnets"))
			}
			if match.GetPort() != 0 {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", tlsMatchFieldPath.Child("Port")), vs)
				klog.Infof("ignoring field: %v", tlsMatchFieldPath.Child("Port"))
			}
			if len(match.GetSourceLabels()) > 0 {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", tlsMatchFieldPath.Child("SourceLabels")), vs)
				klog.Infof("ignoring field: %v", tlsMatchFieldPath.Child("SourceLabels"))
			}
			if len(match.GetGateways()) > 0 {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", tlsMatchFieldPath.Child("Gateways")), vs)
				klog.Infof("ignoring field: %v", tlsMatchFieldPath.Child("Gateways"))
			}
			if match.GetSourceNamespace() != "" {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", tlsMatchFieldPath.Child("SourceNamespace")), vs)
		...
		c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("successfully converted to TLSRoute \"%v/%v\"", tlsRoute.Namespace, tlsRoute.Name), vs)
	}
...
func (c *resourcesToIRConverter) convertVsTCPRoutes(virtualService metav1.ObjectMeta, istioTCPRoutes []*istiov1beta1.TCPRoute, fieldPath *field.Path) []*gatewayv1alpha2.TCPRoute {
	...
	for i, route := range istioTCPRoutes {
		...
		var backendRefs []gatewayv1.BackendRef
		for _, destination := range route.GetRoute() {
			backendObjRef := c.destination2backendObjRef(c.ctx, destination.GetDestination(), virtualService.Namespace, tcpRouteFieldPath)
			if backendObjRef != nil {
				backendRefs = append(backendRefs, gatewayv1.BackendRef{
					BackendObjectReference: *backendObjRef,
					Weight:                 &destination.Weight,
		...
		for j, match := range route.GetMatch() {
			...
			if len(match.GetDestinationSubnets()) > 0 {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", tcpMatchFieldPath.Child("DestinationSubnets")), vs)
				klog.Infof("ignoring field: %v", tcpMatchFieldPath.Child("DestinationSubnets"))
			}
			if match.GetPort() != 0 {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", tcpMatchFieldPath.Child("Port")), vs)
				klog.Infof("ignoring field: %v", tcpMatchFieldPath.Child("Port"))
			}
			if match.GetSourceSubnet() != "" {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", tcpMatchFieldPath.Child("SourceSubnet")), vs)
				klog.Infof("ignoring field: %v", tcpMatchFieldPath.Child("SourceSubnet"))
			}
			if len(match.GetSourceLabels()) > 0 {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", tcpMatchFieldPath.Child("SourceLabels")), vs)
				klog.Infof("ignoring field: %v", tcpMatchFieldPath.Child("SourceLabels"))
			}
			if match.GetSourceNamespace() != "" {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", tcpMatchFieldPath.Child("SourceNamespace")), vs)
				klog.Infof("ignoring field: %v", tcpMatchFieldPath.Child("SourceNamespace"))
			}
			if len(match.GetGateways()) > 0 {
				c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", tcpMatchFieldPath.Child("Gateways")), vs)
		...
		c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("successfully converted to TCPRoute \"%v/%v\"", tcpRoute.Namespace, tcpRoute.Name), vs)
	}
...
func (c *resourcesToIRConverter) isVirtualServiceAllowedForGateway(gateway types.NamespacedName, vs *istioclientv1beta1.VirtualService, fieldPath *field.Path) bool {
	...
	if !isAllowedNamespace {
		c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("gateway from vs.Spec.Gateways %q is not visible in vs.ExportTo %v, parentRefs are not generated for this host, path: %v", gateway.String(), vs.Spec.GetExportTo(), fieldPath), vs)
	...
	if !ok {
		c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("no info about gateway %v allowed hosts, parentRefs won't be generated to it, path: %v", gateway.String(), fieldPath), vs)
	...
	for _, host := range vs.Spec.GetHosts() {
		...
		if ok && matchAny(hosts.UnsortedList(), host) {
			c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("host for gateway \"%v\" matched from same namespace as VirtualService \"%v\", namespace: %v", gateway, vs.Name, vs.Namespace), vs)
			return true
		}

		hosts, ok = allowedHosts["."]
		if ok && vs.Namespace == gateway.Namespace && matchAny(hosts.UnsortedList(), host) {
			c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("host for gateway \"%v\" matched from the current namespace", gateway), vs)
			return true
		}

		hosts, ok = allowedHosts["*"]
		if ok && matchAny(hosts.UnsortedList(), host) {
			c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("host for gateway \"%v\" matched from all namespaces", gateway), vs)
			return true
		}
	}

	c.notifier.Notify(notifications.WarningNotification, fmt.Sprintf("no host in vs.Spec.Hosts matched any gateway.allowedHosts, parentRefs are not generated for this VirtualService, path: %v", fieldPath), vs)
...
func (c *resourcesToIRConverter) generateReferences(vs *istioclientv1beta1.VirtualService, fieldPath *field.Path) ([]gatewayv1.ParentReference, []*gatewayv1beta1.ReferenceGrant) {
	...
	for _, allowedGateway := range vs.Spec.GetGateways() {
		...
		if !ok {
			c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("namespace of \"%v\" gateway taken from namespace of VirtualService", gwName), vs)
		}
		...
		if gateway.Namespace != vs.Namespace {
			...
			c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("successfully created reference grant from %v to %v namespace", vs.Namespace, gateway.Namespace), vs, referenceGrant)
		}

		parentRefs = append(parentRefs, parentRef)
		c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("generated new Parent Reference %v", parentRef.Name), vs)
	}
...
func (c *resourcesToIRConverter) destination2backendObjRef(ctx context.Context, destination *istiov1beta1.Destination, vsNamespace string, fieldPath *field.Path) *gatewayv1.BackendObjectReference {
	vs := ctx.Value(virtualServiceKey).(*istioclientv1beta1.VirtualService)
	if destination == nil {
		c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("destination is nil: %v", fieldPath), vs)
		klog.Infof("destination is nil: %v", fieldPath)
		return nil
	}

	if destination.GetSubset() != "" {
		c.notifier.Notify(notifications.InfoNotification, fmt.Sprintf("ignoring field: %v", fieldPath.Child("Destination", "Subset")), vs)
