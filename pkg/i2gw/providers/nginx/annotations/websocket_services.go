...
func WebSocketServicesFeature(ingresses []networkingv1.Ingress, _ map[types.NamespacedName]map[string]int32, _ *providerir.ProviderIR, notifier *notifications.Notifier) field.ErrorList {
	for _, ingress := range ingresses {
		if webSocketServices, exists := ingress.Annotations[nginxWebSocketServicesAnnotation]; exists && webSocketServices != "" {
			...
			notifier.Notify(notifications.InfoNotification, message, &ingress)
		}
	}
