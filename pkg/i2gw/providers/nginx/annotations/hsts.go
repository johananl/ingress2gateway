...
func HSTSFeature(ingresses []networkingv1.Ingress, _ map[types.NamespacedName]map[string]int32, ir *providerir.ProviderIR, notifier *notifications.Notifier) field.ErrorList {
	...
	for _, rg := range ruleGroups {
		for _, rule := range rg.Rules {
			if hsts, ok := rule.Ingress.Annotations[nginxHSTSAnnotation]; ok && hsts == "true" {
				errs = append(errs, processHSTSAnnotation(rule.Ingress, ir, notifier)...)
			}
		}
	}
...
func processHSTSAnnotation(ingress networkingv1.Ingress, ir *providerir.ProviderIR, notifier *notifications.Notifier) field.ErrorList {
	var errs field.ErrorList
	...
	if maxAge, ok := ingress.Annotations[nginxHSTSMaxAgeAnnotation]; ok && maxAge != "" {
		if _, err := strconv.Atoi(maxAge); err != nil {
			notifier.Notify(notifications.ErrorNotification, "nginx.org/hsts-max-age: Invalid max-age value, must be a number", &ingress)
