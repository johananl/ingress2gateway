...
func TestGetBackendConfigMapping(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			beConfigToSvcs := getBackendConfigMapping(context.TODO(), gceProvider.storage, notifier)
...
func TestGetBackendConfigName(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			notifier := notifications.NewNotifier(notifications.NewNotificationAggregator(), ProviderName)
			gotName, gotExists := getBackendConfigName(ctx, tc.service, tc.beConfigKey, notifier)
