...
func TestPrettyHandlerWithAttrs(t *testing.T) {
	...
	output := buf.String()
	if !strings.Contains(output, "base=attr") {
		t.Errorf("expected output to contain 'base=attr', got %q", output)
	}
	if !strings.Contains(output, "extra=value") {
		t.Errorf("expected output to contain 'extra=value', got %q", output)
	}
}
