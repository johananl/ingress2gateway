...
// Provider returns a slog attribute for the provider name.
func Provider(name string) slog.Attr {
	return slog.String("provider", name)
}

// ObjectRef returns a slog attribute for a Kubernetes object reference.
// The format is "Kind:namespace/name" or "Kind:name" for cluster-scoped objects.
...
// ObjectRefs returns a slog attribute for multiple Kubernetes object references.
