...
func (rm *ResourceManager) Acquire(key string, install InstallFunc) Resource {
	log.Printf("Acquire called for %s", key)
	defer log.Printf("Acquire returned for %s", key)
...
func (rm *ResourceManager) release(key string) <-chan struct{} {
	log.Printf("release called for %s", key)
	defer log.Printf("release returned for %s", key)
